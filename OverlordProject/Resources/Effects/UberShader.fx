...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	float4 sampledNormal = gTextureNormal.Sample(gTextureSampler, texCoord);
...
{
	float3 observedArea = dot(normal, -gLightDirection);
	
	float3 diffuse = gColorDiffuse * saturate(observedArea);

	if(gUseTextureDiffuse)
	{
		float3 textureColor = gTextureDiffuse.Sample(gTextureSampler,texCoord) * observedArea;
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	...
	float absDot = saturate(abs(dotProduct));
	float invFalloff = 1 - absDot;
	float toPower = pow(invFalloff, gFresnelPower);
	float Fresnel = toPower * gFresnelMultiplier;
	
	//Fresnel Mask
	float dotProductMask = dot(float3(0, -1, 0), normal);
	float invertedMask = 1 - saturate(dotProductMask);
	float FresnelMask = pow(invertedMask, gFresnelHardness);
	
	float result = Fresnel * FresnelMask * gColorFresnel;
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	...
	float3 textureSampleReflect = gCubeEnvironment.Sample(gTextureSampler,reflectDir) * gReflectionStrength;
	float3 textureSampleRefract = gCubeEnvironment.Sample(gTextureSampler,refractDir) * gRefractionStrength;
	
	return textureSampleReflect + textureSampleRefract;
}
