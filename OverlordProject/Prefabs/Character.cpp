...
void Character::Initialize(const SceneContext& /*sceneContext*/)
{
	...
	pCamera->GetTransform()->Translate(0.f, m_CharacterDesc.controller.height, -10.f);
...
void Character::Update(const SceneContext& sceneContext)
{
	if (m_pCameraComponent->IsActive())
	{
		...
		//constexpr int ShootMouseButton{ 1 };
		//if (InputManager::IsMouseButton(InputState::down, ShootMouseButton))
		//{
		//	const float speed{ 50.f };
		//	BulletPrefab* Bullet = new BulletPrefab(speed, GetVelocity());
		//
		//	//AddChild(Bullet);
		//
		//	GetScene()->AddChild(Bullet);
		//}

		constexpr int lookAroundMouseButton{ 2 };
		...
		float SensX{ 3.f };
		float SensY{ 2.f };
		if (InputManager::IsGamepadConnected(GamepadIndex::playerOne))
		{
			if (InputManager::GetThumbstickPosition(false).x > deadzone)
			{
				look.x = InputManager::GetThumbstickPosition(false).x + SensX;
			}
			if (InputManager::GetThumbstickPosition(false).x < -deadzone)
			{
				look.x = InputManager::GetThumbstickPosition(false).x - SensX;
			...
			if (InputManager::GetThumbstickPosition(false).y > deadzone)
			{
				look.y = InputManager::GetThumbstickPosition(false).y - SensY;
			}
			if (InputManager::GetThumbstickPosition(false).y < -deadzone)
			{
				look.y = InputManager::GetThumbstickPosition(false).y + SensY;
		...
		GetTransform()->Rotate(m_TotalPitch, m_TotalYaw, 0.0f);
		//********
		//MOVEMENT
		...
		if (abs(move.x) > 0.0f || abs(move.y) > 0.0f || abs(stickX) > 0.0f || abs(stickY) > 0.0f)
		{
			//Calculate & Store the current direction (m_CurrentDirection) >> based on the forward/right vectors and the pressed input
			const XMVECTOR moveDirection{ forward * move.y + right * move.x };
			const XMVECTOR moveDirectionController{ forward * stickY + right * stickX };
			XMStoreFloat3(&m_CurrentDirection, moveDirection);

			if (sceneContext.pInput->IsGamepadConnected(GamepadIndex::playerOne))
			{
				XMStoreFloat3(&m_CurrentDirection, moveDirectionController);
			}
...
void Character::SetVelocity(float Yforce)
{
	m_TotalVelocity.y = Yforce;
}
