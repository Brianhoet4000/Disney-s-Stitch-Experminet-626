...
void ModelAnimator::Update(const SceneContext& sceneContext)
{
	...
	if (m_IsPlaying && m_ClipSet)
	{
		...
		if (passedTicks > durationInTicks)passedTicks = fmodf(passedTicks, m_CurrentClip.duration);
...
void ModelAnimator::SetAnimation(const std::wstring& clipName)
{
	...
	const auto animationToPlay = std::find_if(animations.begin(), animations.end(), [clipName](const AnimationClip& clip) {
		return clip.name == clipName;
		});
	if (animationToPlay == animations.end())
	{
		Reset();
		Logger::LogWarning(L"Cannot find animation with the following name: ", clipName);
	}
	else
	{
